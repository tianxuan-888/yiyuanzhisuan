    ...
    if (user.role === 'provider') {
      ...
      try {
        ...
        if (providerResult && providerResult.length > 0) {
          ...
          // providers表无记录：从 quota_accounts 读取（额度分配时会写入此表）
          const accountResult = await query(
            `SELECT CAST(balance AS FLOAT) as balance_float, CAST(total_in AS FLOAT) as total_in_float
             FROM quota_accounts WHERE user_id = $1`,
            [userId]
          );
          if (accountResult && accountResult.length > 0) {
            quotaData.total_quota = accountResult[0].total_in_float || 0;
            quotaData.available_quota = accountResult[0].balance_float || 0;
            quotaData.used_quota = Math.max(0, quotaData.total_quota - quotaData.available_quota);
          } else {
            // 最终fallback：从 quota_allocations 汇总
        ...
        try {
          ...
          console.error('从quota_accounts fallback读取额度失败:', e2);
        }
      }
