...
export async function POST(request: NextRequest) {
  try {
    ...
    await execute(
      ...
      [crypto.randomUUID(), userId, 'convert', pointsAmount.toFixed(2), newPoints.toFixed(2), `收益转收益产生积分5%: ${pointsAmount}元`]
    );
    ...
    console.error('服务商收益转收益失败:', error);
