  ...
  try {
    ...
    if (marketFee > 0) {
      ...
      if (parentShare > 0 && provId) {
        const { data: provUser } = await client.from('users').select('inviter_id').eq('id', provId).maybeSingle();
        const parentProviderId = provUser?.inviter_id || provId;
